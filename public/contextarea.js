    ...
    class ContextAreaInstance {
        ...
        _initEditor(options) {
            const editorOpts = Object.assign({
                ...
                theme: options.theme ?? (matchMedia('(prefers-color-scheme:dark)').matches ? 'vs-dark' : 'vs'),
                fontSize: 14,
                lineNumbers: 'on',
                wordWrap: 'on',
        ...
        _updateUrlDecorations() {
            ...
            this.urlDecorations = this.editor.deltaDecorations(this.urlDecorations, decorations);
        ...
        _fetchContext(url) {
            if (this.contextCache.has(url)) return Promise.resolve(this.contextCache.get(url));
            if (this.pendingContextFetches.has(url)) return this.pendingContextFetches.get(url);
                        ...
                        for (const [url, positions] of self.currentUrlsInText.entries()) {
                            for (const { range: ur } of positions) {
                                ...
                                actions.push({
                                    ...
                                    kind: 'quickfix',
                                    isPreferred: true,
                                    command: { id: 'expandUrl', title: 'Expand URL', arguments: [url, ur] },
                                });
                            }
                        }
        ...
        async _handleExpandUrl(url, range) {
            ...
            try {
                ...
                this.editor.executeEdits('expand-url', [{ range, text: data.content, forceMoveMarkers: true }]);
        ...
        _setupPasteHandler() {
            ...
            this.editor.getDomNode().addEventListener('paste', () => {
            ...
            this.editor.onDidPaste(async (e) => {
                ...
                if (pastedText.length <= self.config.pasteThreshold || /^https?:\/\//.test(pastedText.trim())) {
                    return;
                }

                // Remove pasted text, restore scroll
                self.editor.executeEdits('paste-intercept', [{ range, text: '', forceMoveMarkers: true }]);
        ...
        _setupDragAndDrop() {
            ...
            overlay.innerHTML = '<div class="ca-drop-content"><div class="icon">\ud83d\udcc1</div><h2>Drop Files Here</h2><p>Files will be uploaded and inserted as links</p></div>';
        ...
        _setupSuggestions() {
            ...
            const suggestObs = new MutationObserver(() => requestAnimationFrame(patchIcons));
            suggestObs.observe(document.body, { childList: true, subtree: true, characterData: true, attributes: true });
            ...
            const detailsObs = new MutationObserver(() => {
                ...
                if (widget && !widget.classList.contains('hidden')) {
                    detailsOpened = true;
                    setTimeout(() => this.editor.trigger('api', 'toggleSuggestionDetails'), 50);
                }
            });
            detailsObs.observe(document.body, { childList: true, subtree: true, attributes: true });
        ...
        _insertTextAtCursor(text) {
            ...
            this.editor.executeEdits('paste', [{ range, text, forceMoveMarkers: true }]);
